...
from pray.forms import PersonForm
from pray.models import Contract, File, Person
...
def add_person(request):
    return create_object(request, form_class=PersonForm,
        post_save_redirect=reverse('pray.views.show_person',
...
def delete_person(request, key):
    return delete_object(request, Person, object_id=key,
        post_delete_redirect=reverse('pray.views.list_people'))
...
def create_admin_user(request):
    ...
    if not user or user.username != 'admin' or not (user.is_active and
        ...
        user.put()
    return render_to_response(request, 'pray/admin_created.html')
